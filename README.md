...
Implements a feedback comb filter — the core building block of reverb. The algorithm is `y[n] = x[n] + g * y[n - D]`, where `g` is feedback gain and `D` is the delay in samples.
